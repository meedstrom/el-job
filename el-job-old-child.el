;;; el-job-old-child.el --- Worker code for children  -*- lexical-binding: t; -*-
...
(defun el-job-old-child--zip (list1 list2)
...
(defun el-job-old-child--work (func &optional _)
  "Handle input from `el-job-old--exec-workload' and print a result.
...
FUNC comes from the :funcall-per-inputs argument of `el-job-old-launch'.
      ...
      (while (setq input (read-minibuffer ""))
        (let (item start output metadata results)
          ...
          (while input
            ...
            (setq results (el-job-old-child--zip output results)))
...
(provide 'el-job-old-child)

;;; el-job-old-child.el ends here
